 ...
 */
int close_errchk(int filedescriptor)
{
	int err;
 ...
 *
 * @filedescriptor1: first descriptor to close
 * @filedescriptor2: second descriptor to close
 * @filename: filename prompting the error
 *
 * Return: 99
 */
int write_err(int filedescriptor1, int filedescriptor2, char *filename)
{
	dprintf(STDERR_FILENO, "Error: Can't write to %s\n", filename);
	close_errchk(filedescriptor1);
	close_errchk(filedescriptor2);
	return (99);
}
 ...
 * Return: 98
 */
int read_err(int filedescriptor1, int filedescriptor2, char *filename)
{
